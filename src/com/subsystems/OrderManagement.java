...
public class OrderManagement implements Subsystems {
    ...
    // ============================================================
    // PURCHASE_REQUESTED
    // TC18, TC21: Handle OrderCreateRequest - stock should be checked before payment
    // ============================================================
    private CompletableFuture<Void> onPurchase(Message m) {
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            if (payload instanceof OrderCreateRequest req) {
                // TC21: Stock validation should happen before payment
                // Note: Stock validation is done in CreateOrderManager or UI layer
    ...
    // ============================================================
    // PAYMENT_AUTHORIZED
    // TC18: Purchase Success - order confirmed after payment
    // ============================================================
    private CompletableFuture<Void> onPaymentAuthorized(Message m) {
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            if (payload instanceof OrderCreateRequest req) {
                try {
                    System.out.println("[OrderManagement] Payment authorized for user " + req.getUserId() + ", creating order...");
                    ...
                    // Create order using CreateOrderManager (creates order + first order item + reduces stock)
                    ...
                    for (int i = 1; i < req.getItems().size(); i++) {
                        ...
                        com.entities.Item item = itemRepo.findById(itemReq.getItemId());
                        if (item == null) {
                            System.out.println("[OrderManagement] Item " + itemReq.getItemId() + " not found, skipping");
                            continue;
                        }
                        
                        // Check stock
                        if (item.getStockQuantity() < itemReq.getQuantity()) {
                            System.out.println("[OrderManagement] Insufficient stock for item " + itemReq.getItemId() + ", skipping");
                            continue;
                        }
                        ...
                        // Reduce stock
                        itemRepo.updateStock(itemReq.getItemId(), item.getStockQuantity() - itemReq.getQuantity());
                    }
                ...
                System.out.println("[OrderManagement] Payment authorized with unknown payload type: " + payload.getClass().getName());
                return;
            }
    ...
    // ============================================================
    // PAYMENT_DENIED
    // TC19, TC20: Payment Processing Timeout/Failure and Payment Rejected
    // ============================================================
    private CompletableFuture<Void> onPaymentDenied(Message m) {
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            // TC19, TC20: Handle payment denial/timeout
            String reason = "Payment Authorization Failed";
            if (payload instanceof String) {
                reason = (String) payload;
            } else if (payload instanceof com.common.dto.payment.PaymentAuthorizeRequest req) {
                reason = "Payment Failed: Authorization denied";
                System.out.println("[OrderManagement] Payment denied for user " + req.getUserId() + ", amount $" + req.getAmount());
            } else if (payload instanceof Order order) {
                ...
                // TC19, TC20: Order status should NOT be set to PLACED
    ...
    // TC23, TC24: Cancel Eligible/Ineligible Order
    // ============================================================
    ...
    private CompletableFuture<Void> onHistoryRequest(Message message) {
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            System.out.println("[OrderManagement] Order history returned for customer " + customerId + ": " + orders.size() + " orders");
        });
    }
}